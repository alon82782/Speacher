...
// ─────────────────────────────────────────────
// Speacher — AuthPage.jsx
// 사용법: <AuthPage onAuth={(user) => console.log(user)} />
// 의존성: React, Tailwind CSS
// index.html <head>에 아래 폰트 추가 필요:
...
export default function AuthPage({ onAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    // TODO: 실제 API 연동 (POST /api/v1/auth/login or /register)
    await new Promise((r) => setTimeout(r, 1200));
    setLoading(false);
    onAuth?.({ email: form.email, name: form.name });
  };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex bg-neutral-900 rounded-2xl p-1 mb-8 border border-neutral-800">
            {["login", "signup"].map((m) => (
              <button
                key={m}
                onClick={() => switchMode(m === mode ? mode : m)}
          ...
          <div className="mb-8">
            ...
            <p className="text-neutral-500 text-sm">
              {isLogin
                ? "계정에 로그인해 이전 분석 결과를 확인하세요."
                : "무료로 시작하고 발표 실력을 키워보세요."}
            </p>
          </div>
